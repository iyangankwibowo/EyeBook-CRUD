            ...
            @foreach ($posts as $post)
                <div class="panel panel-default">
                    ...
                    <div class="panel-heading" style="font-size:17px; font-weight:bold; font-family:Helvetica Neue; background-color:#282929; color:#ffff">
                        <a href="{{ route('post.show', $post) }}" style="font-size=15px">{{ $post->title }}</a>
                        | {{ $post->category->name }}
                        ...
                        <div class="pull-right" style="padding-left:5px">
                            <a href="{{ route('post.show', $post) }}" class="btn btn-xs btn-default">Show Detail</a>
                        </div>
                    ...
                    <div class="panel-body">
                        ...
                        <p style="font-size:20px; font-family:Verdana"> {{ str_limit($post->content, 100,'[ . . . ]') }} </p>
                    </div>
                    ...
                    <div class="panel-body" style="font-family:cursive">
                        ...
                        <p style="font-size:15px; font-family:Verdana"> {{ $comment->message }} </p>
                    </div>
                    @endforeach

                            <form action="{{ route('post.comment.store', $post) }}" method="post" class="form-horizontal">
                            {{ csrf_field() }}
                                <textarea name="message" id="" cols="30" rows="1" class="form-control" placeholder="Write your comment here"></textarea>
